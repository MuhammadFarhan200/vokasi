<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
    ...
    <tbody class="fw-semibold text-gray-600">
        ...
        <tr>
            ...
            <td>
                <a href="{{route('office.civitas.staff.personal.index',$item->id)}}" class="menu-link text-gray-600 text-hover-primary mb-1">{{$item->name}}</a>
            </td>
            <td>
                <a href="{{route('office.civitas.staff.edit',$item->id)}}" class="menu-link text-gray-600 text-hover-primary mb-1">{{$item->email}}</a>
            </td>
            <td>
                @if ($item->is_active == 1)
            ...
            <td class="text-end text-nowrap">
                ...
                @if ($item->is_active == 1)
                <button type="button" title="Non Aktifkan" id="tombol_non_aktif" data-redirect-url="{{route('office.civitas.staff.index')}}" onclick="handle_is_active('PATCH','{{route('office.civitas.staff.update',$item->id)}}','#tombol_non_aktif', 0);" class="btn btn-icon btn-danger"><i class="las la-times fs-2"></i></button>
                @else
                <button type="button" title="Aktifkan" id="tombol_aktif" data-redirect-url="{{route('office.civitas.staff.index')}}" onclick="handle_is_active('PATCH','{{route('office.civitas.staff.update',$item->id)}}','#tombol_aktif', 1);" class="btn btn-icon btn-success"><i class="las la-check fs-2"></i></button>
                @endif
            </td>
        </tr>
        @endforeach
