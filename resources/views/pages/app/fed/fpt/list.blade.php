<table class="table align-middle table-row-dashed fs-6 gy-5" id="fpt_table">
    <thead>
    ...
    <tbody class="fw-semibold text-gray-600">
        ...
        <tr>
            ...
            <td class="text-end text-nowrap">
                @if (request()->is('office/fed/final-project-tester*'))
                    ...
                    <a href="{{ route('office.fed.fpt.edit', $item->id) }}" class="menu-link btn btn-sm btn-primary pe-4 text-nowrap">
                ...
                @endif
                    @else
                    <a href="{{ route('office.frk.fpt.show', $item->id) }}" class="menu-link btn btn-sm btn-primary pe-4 text-nowrap" title="Lihat Detail">
                        <i class="fa-solid fa-circle-info"></i>
                    </a>
                @endif
            </td>
        </tr>
        @include('pages.app.fed.fpt.show')
        @endforeach
    </tbody>
</table>
@if (request()->is('office/fed/final-project-tester*'))
    {{$fpt->links('themes.app.fed_pagination.pagination_fpt')}}
@else
    {{$fpt->links('themes.app.frk_fed_history.pagination_fpt')}}
@endif
