    ...
    <tbody class="fw-semibold text-gray-600">
        ...
        <tr>
            ...
            <td class="text-end text-nowrap">
                @if (request()->is('office/fed/final-project-advisor*'))
                    ...
                    <a href="{{ route('office.fed.fpa.edit', $item->id) }}" class="menu-link btn btn-sm btn-primary pe-4 text-nowrap">
                    ...
                    @endif
                @else
                    <a href="{{ route('office.frk.fpa.show', $item->id) }}" class="menu-link btn btn-sm btn-primary pe-4 text-nowrap" title="Lihat Detail">
                        <i class="fa-solid fa-circle-info"></i>
                    </a>
                @endif
            </td>
        </tr>
        @include('pages.app.fed.fpa.show')
        @endforeach
    </tbody>
</table>
@if (request()->is('office/fed/final-project-advisor*'))
    {{$fpa->links('themes.app.fed_pagination.pagination_fpa')}}
@else
    {{$fpa->links('themes.app.frk_fed_history.pagination_fpa')}}
@endif
