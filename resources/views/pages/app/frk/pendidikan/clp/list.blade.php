    ...
    <tbody class="fw-semibold text-gray-600">
        ...
        <tr>
            ...
            <td class="text-end text-nowrap">
                <button data-bs-toggle="modal" data-bs-target="#clp-{{ $item->id }}" class="menu-link btn btn-sm btn-primary pe-4 text-nowrap">
                    <i class="fa-solid fa-circle-info"></i>
                </button>
                @if (App\Helpers\FedHelper::checkBatasWaktuPengisian($item->year))
                <a href="{{ route('office.frk.college-leadership-position.edit', $item->id) }}" class="menu-link btn btn-sm btn-primary pe-4 text-nowrap">
        ...
        @include('pages.app.frk.pendidikan.clp.show')
        @endforeach
    </tbody>
</table>
{{$collection->links('themes.app.frk_pagination.pagination_teaching')}}
